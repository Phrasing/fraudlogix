#ifndef FRAUDLOGIX_CUDA_SHA256_CUH_
#define FRAUDLOGIX_CUDA_SHA256_CUH_

#include "sha256.h"

__constant__ uint32_t kSha256Constants[64] = {
...
__device__ void Sha256Transform(Sha256Context* ctx, const uint8_t data[]) {
...
__device__ void Sha256Init(Sha256Context* ctx) {
  ctx->datalen = 0;
  ctx->bitlen = 0;
...
__device__ void Sha256Update(Sha256Context* ctx, const uint8_t data[],
                              size_t len) {
...
__device__ void Sha256Final(Sha256Context* ctx, uint8_t hash[]) {
  ...
  for (i = 0; i < 4; ++i) {
    ...
    hash[i + 12] =
        static_cast<uint8_t>((ctx->state[3] >> (24 - i * 8)) & 0xff);
    hash[i + 16] =
        static_cast<uint8_t>((ctx->state[4] >> (24 - i * 8)) & 0xff);
    hash[i + 20] =
        static_cast<uint8_t>((ctx->state[5] >> (24 - i * 8)) & 0xff);
    hash[i + 24] =
        static_cast<uint8_t>((ctx->state[6] >> (24 - i * 8)) & 0xff);
    hash[i + 28] =
        static_cast<uint8_t>((ctx->state[7] >> (24 - i * 8)) & 0xff);
  }
}

#endif  // FRAUDLOGIX_CUDA_SHA256_CUH_
